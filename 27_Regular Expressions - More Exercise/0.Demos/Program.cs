...
namespace _00.Demos
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            using (WebClient client = new WebClient { Encoding = System.Text.Encoding.UTF8 })
            {
                for (int i = 1; i < 25; i++)
                {
                    ...
                    foreach (Match item in matches)
                    {
                        ...
                        Console.WriteLine($"Квартал: {item.Groups["area"].Value}, Цена: {item.Groups["price"].Value}");
                    }
                }
            }
            ...
            foreach (var item in pricesByArea.OrderByDescending(x=>x.Value.Average()))
            {
                Console.WriteLine($"Area: {item.Key}, Average price: {item.Value.Average():F2} EUR, Count: {item.Value.Count}");
            }
        }

        private static string GetHtml(WebClient client, int index)
        {
            byte[] content = client.DownloadData(@$"https://www.imot.bg/pcgi/imot.cgi?act=3&slink=8a7akt&f1={index}");
